...
const handleOpenAi = async (
  ...
  logger.info(`[handleOpenAi:bodyMessage] ${bodyMessage}`);
  ...
  if (msg.message?.conversation || msg.message?.extendedTextMessage?.text) {
    ...
    for (let i = 0; i < Math.min(prompt.maxMessages, messages.length); i++) {
      ...
      if (["extendedTextMessage", "chat"].includes(message.mediaType)) {
        if (message.fromMe) {
...
const flowBuilderQueue = async (
  ...
  console.log(1809, "flowBuilderQueue");
...
const flowbuilderIntegration = async (
  ...
  */
  ...
  if (!msg.key.fromMe && ticket.status === "closed") {
    console.log("===== CHANGE =====");
    await ticket.update({ status: "pending", flowStopped: null });
    await ticket.reload({
      include: [
        { model: Queue, as: "queue" },
        { model: User, as: "user" },
  ...
  if (diferencaEmMilissegundos >= seisHorasEmMilissegundos) {
    ...
    if (flowUse) {
      ...
      // // Enviar as variáveis como parte da mensagem para o Worker
      // const data = {
      //   idFlowDb: flowUse.flowIdNotPhrase,
      //   companyId: ticketUpdate.companyId,
      //   nodes: nodes,
      //   connects: connections,
      //   nextStage: flow.flow["nodes"][0].id,
...
const handleMessage = async (
  ...
  logger.info(`[handleMessage] ${JSON.stringify(msg)}`);
  ...
  try {
    ...
    if (
      ...
      await flowBuilderQueue(ticket, msg, wbot, companyId, contact, isFirstMsg);
    }

    /**
     * Verifica se a mensagem é de um contato que está na fila de atendimento
     */
    if (
      !msg.key.fromMe &&
      !ticket.isGroup &&
