...
import ListSettingsServiceOne from "../SettingServices/ListSettingsServiceOne"; //NOVO TRX SISTEMAS//
import ShowUserService from "../UserServices/ShowUserService"; //NOVO TRX SISTEMAS//
...
const UpdateTicketService = async ({
  ...
  try {
    ...
    if (status !== undefined && ["closed"].indexOf(status) > -1) {
      console.log(118, "UpdateTicketService");
      ...
      console.log(149, "UpdateTicketService");
    ...
    if (settingsTransfTicket?.value === "enabled") {
      ...
      if (oldQueueId !== queueId && oldUserId === userId && !isNil(oldQueueId) && !isNil(queueId)) {
        ...
        if (oldUserId !== userId && oldQueueId === queueId && !isNil(oldUserId) && !isNil(userId)) {
          ...
          const msgtxt = "*Mensaje automático*:\nHa sido transferido al agente *" + nome.name + "*\n¡Espere, en breve lo atenderemos!";
          ...
          if (oldUserId !== userId && !isNil(oldUserId) && !isNil(userId) && oldQueueId !== queueId && !isNil(oldQueueId) && !isNil(queueId)) {
            ...
            const queue = await Queue.findByPk(queueId);
            const nome = await ShowUserService(ticketData.userId);
            const msgtxt = "*Mensaje automático*:\nHas sido transferido al departamento *" + queue?.name + "* y contará con la presencia de *" + nome.name + "*\n¡Espere, en breve lo atenderemos!";
